...
func NewClient(cfg Config) (*PlatformClient, error) {
	opts := []nats.Option{
		...
		nats.DisconnectErrHandler(func(nc *nats.Conn, err error) {
			log.Printf("NATS: соединение разорвано: %v", err)
		}),
		nats.ReconnectHandler(func(nc *nats.Conn) {
			log.Printf("NATS: переподключено к %s", nc.ConnectedUrl())
		}),
		...
		nats.ClosedHandler(func(nc *nats.Conn) {
			log.Printf("NATS: соединение закрыто окончательно")
		}),
	}
	...
	log.Printf("NATS: подключено к %s (JetStream: enabled)", cfg.url())
	return client, nil
}
...
func (p *PlatformClient) initKV(cfg KVConfig) error {
	...
	if err != nil {
		...
		log.Printf("NATS KV: бакет %q уже существует, подключаемся", cfg.BucketName)
		return nil
	}

	log.Printf("NATS KV: бакет %q создан (replicas=%d, history=%d)", cfg.BucketName, cfg.Replicas, cfg.History)
	return nil
}
