...
- [ ] **P-C4** — `internal/services/gateway/handlers.go:325-329` — при ошибке `rand.Read` для sessionID функция возвращается без отправки CloseMessage клиенту. WS обрывается без корректного close-фрейма. Fix: явно отправить CloseMessage или сгенерировать sid до `Upgrade`.

### High

- [ ] **P-H1** — `internal/services/gateway/ratelimit.go:77-84` — `evictOldest` выполняет O(N) линейный скан таблицы IP под `mu`. При DDoS с уникальных IP: каждый новый запрос — скан всего списка под блокировкой → self-amplification. Fix: `container/list` + map (настоящий LRU) или min-heap по `lastSeen`.
- [ ] **P-H2** — `cmd/gateway/main.go:64` — `log.Fatal()` вызывается внутри горутины. `os.Exit(1)` немедленно прерывает процесс, минуя NATS drain и закрытие WS-соединений. Fix: передавать ошибку через канал в main-горутину.
- [ ] **P-H3** — `internal/platform/nc/client.go:336-343` — `Drain` реализован busy-wait циклом `for !Conn.IsClosed() { time.Sleep(50ms) }`. При `MaxAttempts=-1` и зависании NATS может не завершиться до системного kill. Fix: использовать `nats.Conn.Drain()` + `ClosedHandler` (channel notify).
- [ ] **P-H4** — `internal/middleware/xauth.go:87` — ответ собирается конкатенацией: `[]byte(`{"error":"` + text + `"}`)`. Сейчас `text` — хардкод, но паттерн небезопасен и сломается при первом же внешнем вводе. Fix: `json.Marshal` или `strconv.Quote`.
...
- [ ] **I-C1** — `.github/actions/deploy-nomad/action.yml:85` — секреты передаются в `nomad job run` как `-var KEY=VALUE` в командной строке. На Linux это видно через `/proc/<pid>/cmdline` и `ps aux`. `NATS_PASSWORD`, `AUTH_ACCESS_SECRET`, `AUTH_REFRESH_SECRET`, `DATABASE_URL` (включая пароль БД) — все утекают. Fix: экспорт `NOMAD_VAR_<NAME>=value` в env перед `nomad job run` (Nomad понимает эти env vars).
- [ ] **I-C2** — `.github/actions/deploy-nomad/action.yml:37` — `echo "$DEPLOY_SSH_KEY" > /tmp/deploy_key` создаёт файл с дефолтным umask до строки 38 `chmod 600`. На self-hosted runner другой процесс может прочитать ключ в окне гонки. Fix: `(umask 077; echo "$DEPLOY_SSH_KEY" > /tmp/deploy_key)` или `install -m 600 /dev/stdin /tmp/deploy_key <<< "$DEPLOY_SSH_KEY"`.
- [ ] **I-C3** — `.github/workflows/setup.yml:60-69` — секреты (`NATS_CA_KEY`, `NATS_CA_CERT`, `NOMAD_TOKEN`, `NATS_PASSWORD`) интерполируются через `${{ secrets.X }}` напрямую в heredoc. При `ACTIONS_RUNNER_DEBUG=true` или включённом step-debug маскирование может не срабатывать. Fix: использовать `env:` секцию шага, в heredoc ссылаться через `$VAR`.
...
- [ ] **I-M9** — `deployments/envs/prod/setup.sh:39` принимает `INSTALL_POSTGRES`, но блок установки отсутствует. `.github/workflows/setup.yml:68` передаёт эту переменную. Fix: либо реализовать, либо удалить параметр.
