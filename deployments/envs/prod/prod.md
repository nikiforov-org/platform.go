...
CI автоматически подхватит его при следующем push — `for svc in $(ls cmd/)`.
...
Поля: `node_ip` (публичный IP ноды), `platform_domain`, `install_postgres`.
...
for f in /opt/platform/deployments/infra/nomad/*.nomad; do
  nomad job run -var VERSION=v1.2.3 ... $f
done
```

При первом деплое новой архитектуры: остановить старые Nomad-джобы:
```bash
