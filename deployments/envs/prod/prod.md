...
- CA-ключ хранится только в GitHub Secret `NATS_CA_KEY`. На серверах его нет ни на
...
- Сертификат ноды подписан CA, ключ CA хранится только в GitHub Secret
  `NOMAD_CA_KEY`. На серверах его нет ни на постоянном диске: `setup.sh`
...
Команда выдаст `Secret ID` — это management-токен с полными правами. Сохранить в GitHub Secret `NOMAD_TOKEN` и в безопасное хранилище (1Password, Vault).

**Если токен уже есть в GitHub Secrets:**

`Settings → Secrets and variables → Actions → Secrets → NOMAD_TOKEN` — значение и есть bootstrap-токен.
...
**Шаг 3:** В правом верхнем углу нажать **"Sign In"** → вставить токен из `NOMAD_TOKEN`.
...
export NOMAD_TOKEN=<значение из GitHub Secret>
nomad status
...
**ВАЖНО:** все старые токены (включая CI/CD) станут невалидными. Обновить `NOMAD_TOKEN` в GitHub Secrets сразу после bootstrap.
